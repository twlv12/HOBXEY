...
cpdef isValid(int x, int y, world):
...
cdef class Connection:
	cdef public Neuron source, target
	cdef public double weight
	def __init__(self, Neuron source, Neuron target, str weight):
		self.source = source
		self.target = target
...
cdef class Neuron:
	cdef public double inputValue, outputValue
	cdef public str type
	cdef public Creature creature
	cdef public World world
	cdef public list connectionsOut, connectionsIn
	def __init__(self, str type, Brain brain):
		self.inputValue = 0.0
	...
	cpdef connect(self, Neuron target, str weight):
	...
	cpdef public double RandomInput(self, input):
		return random.random()

	cpdef public double BlockageUp(self, input):
		try:
			if self.creature.getCell(self.creature.x, self.creature.y+1).creature: return 1
			else: return 0
		except: return 1

	cpdef public double BlockageDown(self,input):
		try:
			if self.creature.getCell(self.creature.x, self.creature.y-1).creature: return 1
			else: return 0
		except: return 1

	cpdef public double BlockageLeft(self,input):
		try:
			if self.creature.getCell(self.creature.x-1, self.creature.y).creature: return 1
			else: return 0
		except: return 1

	cpdef public double BlockageRight(self,input):
		try:
			if self.creature.getCell(self.creature.x+1, self.creature.y).creature: return 1
			else: return 0
		except: return 1

	cpdef public double XGradient(self, input):
		gradient = 2 * (self.creature.x / worldSize[0] - 0.5)
		return gradient
	   
	cpdef public double YGradient(self, input):
		gradient = 2 * (self.creature.y / worldSize[1] - 0.5)
		return gradient

	cpdef public double ChangeX(self, input):
		try:
			if self.creature.x > self.creature.lastX: return 1
			elif self.creature.x < self.creature.lastX: return -1
		except: return 0

	cpdef public double ChangeY(self, input):
		try:
			if self.creature.y > self.creature.lastY: return 1
			elif self.creature.y < self.creature.lastY: return -1
		except: return 1

	cpdef public double Density(self, input):
		return self.creature.getPopDensity()
	   
	cpdef public double ConstantPos(self, input):
		return 1

	cpdef public double ConstantNeg(self, input):
		return -1

	cpdef public double Invert(self, input):
		return -input

	cpdef public double Multiply(self,input):
		try:
	...
	cpdef public double MoveX(self, input):
		if input > 0:
	...
	cpdef public double MoveY(self, input):
		if input > 0:
	...
	cpdef public double MoveRandom(self, input):
		if input <= -0.2 or input >= 0.2:
			cell = random.choice(self.creature.getAdjacentCells())
			self.creature.moveToCell(cell)
		
	cpdef public double Pheromone(self, input):
		return self.creature.getPheromoneDensity()[0]

	cpdef public double MoveToPheromone(self, input):
		if input >= 0.3 or input <= -0.3:
...
cdef class Brain:
	cdef public Creature creature
	cdef public list neurons, genes
...
cdef class Cell:
	cdef public int x, y
	cdef public object creature
	cdef public float pheromone
...
cdef class Creature:
	cdef public int x, y, lastX, lastY
	cdef public object cell, world, brain
	cdef public str genome
	cdef public float density
	cdef public bint canReproduce
	...
	cdef getPopDensity(self):
		cdef int i, j
		cdef list cells, nearbyCreatures
	...
	cdef getPheromoneDensity(self):
		cdef list adjacent_cells, pheromones
	...
	cdef getAdjacentCells(self):
		cdef int i, j
		cdef list adjacent_cells
		adjacent_cells = []
...
cdef class World:
	cdef public int sizeX, sizeY, movements, population
	cdef public list cells, creatures, freeCells
	cdef public object screen
	cdef public tuple cellSize
